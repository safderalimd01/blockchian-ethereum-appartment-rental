...
import apartment_Abi_address from './abi_contract_adress';
...
class App extends Component{
  ...
  async componentDidMount(){
    ...
    const manager =  await apartment_Abi_address.methods.contractOwnerAddress()
    .call()
    // .send({
    //   from:account[0],
    //   to:contractor
    ...
    const apartment_numbers = await apartment_Abi_address.methods.getApartments().call();
    const listItems = apartment_numbers.map((number) =>
      <li key={number.toString()}>
        {number}
      </li>
    );
    const contractor_balance = await web3.eth.getBalance(apartment_Abi_address.options.address);
    this.setState({
      manager,
      Owner,
      contractor_balance,
  ...
  onSubmit = async (event) => {
    ...
    const units_identifier = await apartment_Abi_address.methods.getUnit(this.state.value).call();
    this.setState({
  ...
  onUnitSubmit= async (event) => {
    event.preventDefault();
    const units_identifier2 = await apartment_Abi_address.methods.unitNumbers(this.state.value2).call();
    this.setState({
  ...
  onSetUnitSubmit= async (event) => {
    ...
    const contractor =  await apartment_Abi_address.options.address;
    const units_identifier2 = await apartment_Abi_address.methods.setUnitOwner(this.state.value3, this.state.value4, false, this.state.value5).send({
        from:accounts[0],
        to:contractor
      });
  ...
  onAssignTenantSubmit = async (event) => {
    ...
    const units_identifier = await apartment_Abi_address.methods.getUnit(this.state.value6).call();
    const contractor =  await apartment_Abi_address.options.address;

    const Assign_tenant_data = await apartment_Abi_address.methods.AssignTenants(this.state.value6, this.state.value7).send({
